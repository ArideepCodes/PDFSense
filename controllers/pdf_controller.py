        ...
        if uploaded_file is not None:
            ...
            if st.button("Convert File", type="primary"):
                with st.spinner("Converting your file..."):
                    try:
                        # Check for poppler if conversion is to PNG
                        if conversion_type == "PDF to PNG Images" and shutil.which("pdftoppm") is None:
                            st.warning("Poppler is not installed on your system. PNG conversion might fail. Please install Poppler and add it to PATH.")
